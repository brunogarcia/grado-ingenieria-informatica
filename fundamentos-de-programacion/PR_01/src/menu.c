...
tError readBook(tBook *book) {
	...
	/******************** PR1 - EX 2B ********************/
	unsigned short tempIsAvailable;

	printf("ISBN (13 digits): \n>> ");
	scanf("%s", book->isbn);
 
	printf("Publication year: \n>> ");
	scanf("%hu", &book->publicationYear);
	
	printf("Is it available?: \n (1 yes, 0 no) >> ");
	scanf("%hu", &tempIsAvailable);
	if (tempIsAvailable == 0) {
	...
	fflush(stdin);
	while(getchar()!='\n');
	book->classification.section = getchar();
	
	printf("Subsection (1 digit): \n>> ");
	fflush(stdin);
	while(getchar()!='\n');
	book->classification.subSection = getchar();
